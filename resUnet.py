...
def residualDownsample(in_data, filters, cropping=None, last=False,
    ...
    with variable_scope(name, reuse=reuse):
        ...
        conv1 = conv2d(in_data, filters, 3)
        conv2 = conv2d(conv1, filters, 3)
        identity = Cropping2D(cropping=2)(in_data)
        bottle=conv2d_linear(identity, filters, 1)
        identityShortcut=bottle+conv2
        #identityShortcut = concat([identity, conv2], axis=3)
...
def residualUpsample(in_data, crop, filters, name='UpSample', reuse=False):
    ...
    with variable_scope(name, reuse=reuse):
        ...
        bottle=conv2d_linear(merge6, filters, 1)
        conv2 = conv2d(merge6, filters, 3)
        conv3 = conv2d(conv2, filters, 3)
        identity = Cropping2D(cropping=2)(bottle)
        identityShortcut = identity+conv3
